...
def get_url(image):
    ...
    token = "token"  ## 自行去github生成token，不懂得文章最好会写
    curr_time = datetime.datetime.now()
    path = curr_time.strftime("%Y%m%d%H%M%S") + img_ext
    #url = "https://api.github.com/repos/foxfur-ry/Images/contents/ouzhenqing/" + path  # 用户名、库名、路径
    headers = {
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            #img = f"https://raw.githubusercontent.com/foxfur-ry/Images/refs/heads/main/ouzhenqing/{path}"
            print(img)
            return img
        else:
